<template>
    <div>
        <div class="J-main-body ">
          <div class="homeMain-header"> 
            <div class="J-main-search" >
              <el-form :inline="true" :model="searchForm" class="demo-form-inline" ref="searchForm">
                ...
                <el-form-item label="专线名称" prop="transferName">
                    <el-input v-model="searchForm.transferName" placeholder="专线名称"></el-input>
                </el-form-item>
          ...
          <div class="J-elTable" id="J-elTable" >
            <el-table  :max-height="getScreenHeight-jElTableOffsetTop>=111?getScreenHeight-jElTableOffsetTop:'none'"  :data="tableData" border  @selection-change="handleSelectionChange" >
                  ...
                  <div class="table-menu">
                    <el-menu  :collapse="true"  >
                      <el-submenu index="1" >
                        ...
                        <el-menu-item-group> 
                          <el-menu-item index="1-1" v-if="scope.row.balanceStatus==1 && checkHasPermission('zxyfjs:update')" @click="showModifyBalance(scope.row)">修改对账单</el-menu-item>  
                          <el-menu-item index="1-2" v-if="scope.row.balanceStatus==1 && checkHasPermission('zxyfjs:delete')" @click="deleteBalance(scope.row)">删除对账单</el-menu-item> 
              ...
              <el-table-column prop="realBalanceName" label="结算人" width="180" align='center'></el-table-column>
              ...
              <!-- <el-table-column prop="sumInFee" label="应收金额" width="180" align='center' class-name="J-income"></el-table-column> -->
              <!-- <el-table-column prop="sumOutFee" label="应付金额" width="180" align='center' class-name="J-expenditure"></el-table-column> -->
              <el-table-column prop="sumDfXxFee" label="信息费" width="80" align='center' class-name="J-income"></el-table-column>
              ...
              <el-table-column prop="sumYjFee" label="月结" width="80" align='center' class-name="J-expenditure"></el-table-column>
              <el-table-column prop="sumException" label="异常费" width="80" align='center' class-name="J-income"></el-table-column>
              ...
              <el-table-column prop="createEmployeeName" label="收款人"  align='center'></el-table-column>
        ...
        <zxCreateBalance @reloadList="reloadList" :pVisible.sync='modifyDialog.visible' pAuFlag='1' 
          :pBalanceId.sync='modifyDialog.balanceId'  :pBalanceNo.sync='modifyDialog.balanceNo'
          :pTransferId.sync='modifyDialog.transferId' :pTransferName.sync='modifyDialog.transferName' v-if="modifyDialog.visible"></zxCreateBalance>
...
import detailZxBalance from './detail.vue'
...
export default {
  ...
  methods: {
    ...
    querySpecialList() {//获取页面初始数据
      api.post('/finance/balance/zx/balance/list/pc', this.searchForm).then(data=>{
          if(data.code==="200"){
    ...
    async showSubmitBalanceAudit(row) {
      ...
      //加载原审核流程信息
      const { code, message, data } =await api.get('/finance/balance/zx/balance/audit/oldflow?id='+row.id)
      if(code=='200'){
    ...
    submitBalanceAudit() {
      ...
      api.post('/finance/balance/zx/balance/audit/submit', this.auditForm).then(data=>{
        if(data.code==="200"){
    ...
    deleteBalance(row){
      this.$confirm('确定要删除对账单['+row.balanceNo+']吗?', '提示', {
        ...
        const { code, message, data } =await api.get('/finance/balance/zx/balance/delete?id='+row.id)
        if(code==200){
          this.$message({
            type: 'success',
            message: '对账单删除成功!'
    ...
    exportFile(row){
      ...
      api.get('/finance/balance/zx/export/pc/list?id='+row.id).then(data=>{
        if (data.code == '200') {
