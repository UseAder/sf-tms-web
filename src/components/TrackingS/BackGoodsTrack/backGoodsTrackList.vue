                ...
                <div class="J-main-search" >
                    <el-form :inline="true" :model="searchForm" class="demo-form-inline" ref="searchForm">
                        ...
                        <el-form-item label="运单状态" prop="status">
                         ...
                         <el-form-item label="退货状态" prop="backGoodsStatus">
                            <el-select v-model="searchForm.backGoodsStatus"  multiple collapse-tags clearable placeholder="请选择" class="J-width100">
                                <el-option
                                    v-for="item in base.queryBackGoodsStatus"
                                    :key="item.value"
                                    :label="item.code"
                                    :value="item.value">
                                </el-option>
                        ...
                        <el-form-item class="J-main-search-button">
                            <el-button type="primary" v-if="checkHasPermission('back_truck:read')" @click='handlesearch'>查询</el-button>
                            <el-button type="warning" v-if="checkHasPermission('back_truck:read')" @click="resetForm('searchForm')">重置</el-button>
                        </el-form-item>
                    </el-form>
                </div>
            </div>
            <div :class="trackWaybillVisible?'visibleWaybillList':''">
                <div class="J-elTable-right J-inputWidth">
                    ...
                    <div class="J-elTable-right-waybill">
                        <v-backGoodsTrackWaybill @upData="updateData" :waybillNo.sync='chooseData.waybillNo' :waybillId.sync='chooseData.waybillId' :convertWaybillStatus.sync='chooseData.waybillStatus'  :backId.sync='chooseData.backId'></v-backGoodsTrackWaybill>
                    </div>
                </div>
                <div class="J-elTable-left" id="J-elTable" >
                    <el-table :max-height="getScreenHeight-jElTableOffsetTop>=111?getScreenHeight-jElTableOffsetTop:'none'"  ref="defaultChoiceData" highlight-current-row :data="tableData" border @current-change="currentChangeFun">
                                ...
                                <el-submenu index="1" >
                                    ...
                                    <el-submenu index="2">
                                        ...
                                        <div class="J-flexW" style="width:156px;padding: 0 5px;">
                                            <div v-for="item in managementMark.managementMarkList" :key="item.id" @click="mark(item.id, scope.row,4)">
                                                <el-tooltip 
                                                    class="item"
                                                    effect="dark"
                                                    :content="item.memo"
                       ...
                       <el-table-column label="标记" min-width="80" align="center">
                        ...
                        <el-table-column prop="" label="跟踪标记" min-width="110" align='center'>
                             <template  slot-scope="scope">
                                <span :class="scope.row.today == true ? 'J-markColor' : 'J-gray' ">今</span>
                                <span :class="scope.row.yesterday == true ? 'J-markColor' : 'J-gray   '">昨</span>
                            </template>
                        </el-table-column>
                        <el-table-column  prop="status" label="退货状态" min-width="110" align='center'>
                            <template slot-scope="scope">
                        ...
                        <el-table-column prop="beginSiteName" label="发站" width="150" align='center'></el-table-column>
                        ...
                        <el-table-column prop="packWeight" label="打包重量" min-width="110" align='center'></el-table-column>
                        <el-table-column prop="packBulk" label="打包体积" min-width="110" align='center'></el-table-column>
                        <el-table-column prop="" label="退货率" min-width="110" align='center'>
                            <template  slot-scope="scope">
                                {{showBackGoodsRate(scope.row)}}
                            </template>
                        </el-table-column>
                        <el-table-column prop="linkman" label="承运人" min-width="110" align='center'></el-table-column>
                        <el-table-column prop="linkmanPhone" label="联系电话" min-width="120" align='center'></el-table-column>
                        <el-table-column prop="transferName"  label="退货承运方" min-width="110" align='center'></el-table-column>
                        <el-table-column prop="transferNo" label="退货承运单号" min-width="110" align='center'></el-table-column>
                        <el-table-column prop="memo" label="退货备注" min-width="110" align='center'></el-table-column>
                    </el-table>
...
import backGoodsTrackWaybill from './BackGoodsTrackWaybill/backGoodsTrackWaybill.vue'
import waybillNoComponent from "@/components/Common/WaybillDetails/waybillNoComponent.vue";

export default {
    name:'t-b-backGoodsTrackList',
    mixins: [Base,List,Track],
    components: { 
       'v-backGoodsTrackWaybill':backGoodsTrackWaybill,
    ...
    methods: {
        ...
        currentChangeFun(currentRow, oldCurrentRow){
            if(currentRow){
                ...
                this.chooseData.backId=currentRow.backId
            }
                
        },
        async querySpecialList() {//获取页面初始数据    
            const { code, message,data } =await api.post('/goods/track/back/goods/list', this.searchForm)
            if(code==="200"){
                if(data.records){
