<template>
    <div>
        <div class="J-main-body ">
          ...
          <div class="J-elTable" id="J-elTable">
            <el-table  :max-height="getScreenHeight-jElTableOffsetTop>=111?getScreenHeight-jElTableOffsetTop:'none'"  :data="tableData" border>
                  ...
                  <div class="table-menu">
                    <el-menu  :collapse="true"  >
                      <el-submenu index="1" >
                        ...
                        <el-menu-item-group> 
                          <el-menu-item index="1-1" v-if="scope.row.balanceStatus==1 &&  checkHasPermission('khyfjs:update')" @click="showModifyBalance(scope.row)">修改对账单</el-menu-item>  
                          <el-menu-item index="1-2" v-if="scope.row.balanceStatus==1 && checkHasPermission('khyfjs:delete')" @click="deleteBalance(scope.row)">删除对账单</el-menu-item> 
                          ...
                          <el-menu-item index="1-5" v-if="scope.row.auditStatus==2 && checkHasPermission('khyfjs:trick')" @click="addKpShow(scope.row)">开票申请</el-menu-item>
                          <el-menu-item index="1-6" v-if="checkHasPermission('khyfjs:out')" @click="exportFile(scope.row)" >导出</el-menu-item>  
                        </el-menu-item-group>
                      </el-submenu> 
                    </el-menu>
                  </div>
              ...
              <el-table-column prop="sumXfFee" label="现付" width="100" align='center' class-name="J-income"></el-table-column>
              <el-table-column prop="sumHfFee" label="回付" width="100" align='center' class-name="J-income"></el-table-column>
              <el-table-column prop="sumYjFee" label="月结" width="100" align='center' class-name="J-income"></el-table-column>
              <el-table-column prop="sumException" label="异常费" width="100" align='center' class-name="J-income"></el-table-column>
              <el-table-column prop="memo" label="对账备注" min-width="180" align='center'></el-table-column>
              <el-table-column prop="createEmployeeName" label="对账人" width="100" align='center'></el-table-column>
        ...
        <!-- 提交开票提示框 -->
        <el-dialog :title="kpDialog.title"
            :visible.sync="kpDialog.visible"
            width="40%"
            center
            :modal=false
            :close-on-click-modal=false
            @close="kpDialog.visible = false" v-if="kpDialog.visible" class="J-el-dialog__body-bot">
            <el-form label-width="90px">
                <el-form-item label="开票备注：">
                    <el-input type="text" placeholder="备注" v-model="kpForm.memo"></el-input>
                </el-form-item>
                <el-form-item label="审核人：">
                    <v-auditorSelect @auditorSelect='selectKpAuditEmployee'></v-auditorSelect> 
                </el-form-item>
            </el-form>
            <span slot="footer" class="dialog-footer" >
                <el-button @click="kpDialog.visible = false">取 消</el-button>
                <el-button type="primary" @click="submitKpAudit" :loading="savingFlag">确 定</el-button>
            </span>
        </el-dialog>
        ...
        <createConSignorBalance @reloadList="reloadList" :pVisible.sync='modifyDialog.visible' pAuFlag='1' 
            :pBalanceId.sync='modifyDialog.balanceId'  :pBalanceNo.sync='modifyDialog.balanceNo'
            :pConsignorId.sync='modifyDialog.consignorId' :pConsignorName.sync='modifyDialog.consignorName'  v-if="modifyDialog.visible"></createConSignorBalance>
...
import createConSignorBalance from './createBalance.vue'
import detailConsignorBalance from './detail.vue'
import auditorSelect from '@/widget/auditorSelect'
export default {
  mixins: [Base,List],
  components: { 
    'v-auditorSelect':auditorSelect,
    'createConSignorBalance':createConSignorBalance,
  ...
  methods: {
    ...
    //开票审核人
    selectKpAuditEmployee(employeeList){
       var eIds = employeeList.map( (v,i) => v.id)
      this.kpForm.auditFlowEmployeeIds = eIds
    },
    ...
    querySpecialList() {//获取页面初始数据
      api.post('/finance/balance/consignor/balance/list/pc', this.searchForm).then(data=>{
          if(data.code==="200"){
    ...
    async showSubmitBalanceAudit(row) {
      ...
      //加载原审核流程信息
      const { code, message, data } =await api.get('/finance/balance/consignor/balance/audit/oldflow?id='+row.id)
      if(code=='200'){
    ...
    submitBalanceAudit() {
      ...
      api.post('/finance/balance/consignor/balance/audit/submit', this.auditForm).then(data=>{
        if(data.code==="200"){
    ...
    deleteBalance(row){
      ...
      this.$confirm('确定要删除对账单['+row.balanceNo+']吗?', '提示', {
        ...
        const { code, message, data } =await api.get('/finance/balance/consignor/balance/delete?id='+row.id)
        if(code==200){
          this.$message({
            type: 'success',
            message: '对账单删除成功!'
     ...
     */
    addKpShow(row) {
      this.kpForm.balanceId = row.id
      this.kpDialog.title = '开票对账单-' + row.balanceNo
      this.kpDialog.visible = true
    },
    submitKpAudit(){

      this.savingFlag = true
      api.post('/finance/balance/consignor/balance/kp/submit', this.kpForm).then(data=>{
        if(data.code=="200"){
          this.$message({type: 'success',message: '提交对账单开票成功!'});
          this.querySpecialList()
          this.kpDialog.visible = false
        }
      }).then(()=>{
        this.savingFlag = false;
      })
    ...
    exportFile(row){
      api.get('/finance/balance/consignor/export/pc/list?id='+row.id).then(data=>{
        if (data.code == '200') {
