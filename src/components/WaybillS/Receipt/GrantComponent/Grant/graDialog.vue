            ...
            <div class="J-detail "> 
                ...
                <el-form :model="searchForm"  label-width="82px" class="demo-ruleForm J-flexRc " >
                    <el-form-item label="清单备注：" style="margin-top:20px">
                        <div class="J-flex">
                            <el-input autosize v-model="newModifyForm.memo" placeholder="清单备注" type="textarea" style="width: 400px;margin-right: 25px;"></el-input>
                            <el-button type="primary" @click='preservation' style="height:31px;line-height:0px">保存清单</el-button>
                        </div>
                    </el-form-item>                      
                </el-form> 
            </div>
...
export default {
    ...
    methods:{
        ...
        //新增 table 数据获取
        async newCreated(){
            this.searchForm.consignorId=this.updateRow.consignorId
            const { code, message,data } =await api.post('/receipt/wait/detail/back/list', this.searchForm)
            if(code==="200"){
                this.tableData = data;
            }
        },
        async preservation(){
            ...
            this.newModifyForm.consignorId=this.updateRow.consignorId
            this.newModifyForm.consignorName=this.updateRow.consignorName
            const { code, message,data } =await api.post('receipt/create/back/pc ', this.newModifyForm)
            if(code==="200"){
                this.$message({
                    type: 'success',
