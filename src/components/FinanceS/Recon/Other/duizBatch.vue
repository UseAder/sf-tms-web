        ...
        <div class="J-main-body">
          ...
          <div class="J-elTable" id="J-elTable" >
            <el-table  :max-height="getScreenHeight-jElTableOffsetTop>=111?getScreenHeight-jElTableOffsetTop:'none'"  :data="tableData" border  @selection-change="handleSelectionChange">
                  ...
                  <div class="table-menu">
                    <el-menu  :collapse="true"  >
                      <el-submenu index="1" >
                        ...
                        <el-menu-item-group> 
                          <el-menu-item index="1-2" v-if="scope.row.balanceStatus==1 && checkHasPermission('qtyfjs:delete')" @click="deleteBalance(scope.row)">删除对账单</el-menu-item> 
                          ...
                          <el-menu-item index="1-6" v-if="checkHasPermission('qtyfjs:read')" @click="exportFile(scope.row)">导出</el-menu-item> 
                        </el-menu-item-group>
                      </el-submenu> 
                    </el-menu>
                  </div>
              ...
              <el-table-column prop="fee" label="费用" min-width="110" align='center'  class-name="J-income"></el-table-column>
              ...
              <el-table-column prop="memo" label="对账备注" min-width="230" align='center'  show-overflow-tooltip></el-table-column>
...
import detailOtherBalance from './detail.vue'
export default {
  mixins: [Base,List],
  components: { 
  ...
  methods: {
    ...
    querySpecialList() {//获取页面初始数据
      api.post('/finance/balance/other/balance/list/pc', this.searchForm).then(data=>{
        if(data.code==="200"){
    ...
    async showSubmitBalanceAudit(row) {
      ...
      //加载原审核流程信息
      const { code, message, data } = await api.get('/finance/balance/other/balance/audit/oldflow?id='+row.id)
      if(code=='200'){
    ...
    submitBalanceAudit() {
      ...
      api.post('/finance/balance/other/balance/audit/submit', this.auditForm).then(data=>{
        if(data.code==="200"){
    ...
    deleteBalance(row){
      this.$confirm('确定要删除对账单['+row.balanceNo+']吗?', '提示', {
        ...
        const { code, message, data } =await api.get('/finance/balance/other/balance/delete?id='+row.id)
        if(code==200){
          this.$message({
            type: 'success',
            message: '对账单删除成功!'
     ...
     exportFile(row){
      api.get('/finance/balance/other/export/pc/list?id='+row.id).then(data=>{
        if (data.code == '200') {
