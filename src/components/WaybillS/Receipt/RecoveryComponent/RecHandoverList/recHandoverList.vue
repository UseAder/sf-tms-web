        ...
        <div class="J-main-body">
            ...
            <div class="J-elTable" id="J-elTable" >
                <el-table  :max-height="getScreenHeight-jElTableOffsetTop>=111?getScreenHeight-jElTableOffsetTop:'none'"  :data="tableData" border >
                    ...
                    <el-table-column label="操作"  width="50" align='center' header-align="center" fixed>
                        ...
                        <div class="table-menu">
                            ...
                            <el-submenu index="1" >
                                ...
                                <el-menu-item-group> 
                                <el-menu-item index="1-2"  v-if="checkHasPermission('receipt:read')" @click="newFun(scope.row)">回单明细</el-menu-item>  
                                <el-menu-item index="1-3"  v-if="checkHasPermission('receipt:delete')" @click="deleteConfirm(scope.row)">删除</el-menu-item>  
                    ...
                    <el-table-column prop="targetName" label="承运方" min-width="110" align='center'></el-table-column>
...
import isDialog from './recMxDialog.vue'
export default {
    mixins: [Base,List],
    components: { 
        'components':isDialog
    ...
    methods: {
        deleteConfirm(row){
            ...
            type: 'warning'
                ...
                const { code, message,data } =await api.post('/receipt/delete/receipt/pc',row.id)
                if(code==200){
                    this.$message({
                    type: 'success',
                    message: '恭喜您，删除成功!'
        ...
        async querySpecialList() {//获取页面初始数据    
          const { code, message,data } =await api.post('/receipt/receipt/pc', this.searchForm)
          if(code==="200"){
