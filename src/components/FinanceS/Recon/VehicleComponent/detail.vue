                ...
                <el-row  style="margin-top:10px" class="J-elTable"> 
                    <el-tabs type="card">
                        <el-tab-pane label="运单清单" >
                            <el-table   :max-height="getScreenHeight-jElTableOffsetTop-200>80?getScreenHeight-jElTableOffsetTop-200:'none'" show-summary :summary-method="zcjsDetailSummaries" :data="dispatchList" border  style="margin-top:15px" >
                                <el-table-column fixed type="index" width="50" label="序号" align='center'></el-table-column> 
                                <el-table-column prop="waybillNo" label="运单号" min-width="150" align='center'>
                                ...
                                <el-table-column prop="dfFee" label="到付" width="100" align='center' class-name="J-expenditure"></el-table-column>
                                <el-table-column prop="xfFee" label="现付" width="100" align='center' class-name="J-expenditure"></el-table-column>
                                <el-table-column prop="hfFee" label="回付" width="100" align='center' class-name="J-expenditure"></el-table-column>
                                <el-table-column prop="yajFee" label="押金" width="100" align='center' class-name="J-expenditure"></el-table-column>
                                <el-table-column prop="exceptionFee" label="异常费" width="100" align='center' class-name="J-expenditure"></el-table-column>       
                                <el-table-column prop="goodsName" label="货物名称" min-width="100" align='center'></el-table-column>       
                                <el-table-column prop="pack" label="包装" width="80" align='center'></el-table-column>
                                <el-table-column prop="goodsNum" label="件数" width="80" align='center'></el-table-column>
                                <el-table-column prop="goodsWeight" label="重量" width="80" align='center'></el-table-column>
                                <el-table-column prop="goodsBulk" label="体积" width="80" align='center'></el-table-column>
...
export default {
    ...
    methods:{
        ...
        async loadBalanceDetail(){
            const { code, message,data } =await api.get('/finance/balance/cart/detail?id='+this.pBalanceId)
            if(code==='200'){
                this.detail=data
        ...
        async dispatchListFun(){//获取信息--运单列表
            const { code, message,data } =await api.get('/finance/balance/cart/waybill/list?id='+this.pBalanceId)
            if(code==='200'){
                this.dispatchList=data
            }
        },
        async dzListFun(){//获取信息--对账审核过程
            const { code, message,data } =await api.get('finance/balance/cart/dz/audit/list?id='+this.pBalanceId)
            if(code==='200'){
                this.dzList=data
            }
        },
        async hxListFun(){//获取信息--核销记录
            const { code, message,data } =await api.get('/finance/balance/cart/hx/list?id='+this.pBalanceId)
            if(code==='200'){
                this.hxList=data
            }
        },
         ////////////// 合计  //////////////////////////////
        zcjsDetailSummaries(param) {//合计
            ...
            const arr=['dfFee','xfFee','hfFee','yajFee','exceptionFee']
