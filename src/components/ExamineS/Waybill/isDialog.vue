            ...
            <div class="J-detail  J-inputBorder0"> 
                ...
                <el-row>
                    ...
                    <el-table :max-height="250"  :data="wbDetail" border  style="width: 100%">
                        ...
                        <el-table-column prop="auditEmployeeName" label="审核人" width="180"  align='center' header-align="center"></el-table-column>
...
export default {
    ...
    methods:{
        ...
        init(){
            if(this.isContent){
                this.newModifyTitle= this.updateRow.waybillNo+'运单变更审核'
                this.closeOnClickModal=false
            }else{
                this.newModifyTitle= this.updateRow.waybillNo+'运单变更审核详情'
        ...
        async wbDetailFun(){
            const { code, message,data } =await api.get('/audit/wb/detail?auditId='+this.updateRow.id)
            if(code==="200"){
                this.wbDetail = data;
            }
        },
        async wbChangeFun(){
            const { code, message,data } =await api.get('/audit/wb/change?auditId='+this.updateRow.id)
            if(code==="200"){
                this.wbChange = data;
            }
        },
        ...
        //审核意见确认提交
        async auditWbPs(){
            const { code, message,data } =await api.post('audit/wb/ps',this.newModifyForm)
            if(code==200){
                this.$message({
                    type: 'success',
                    message: '审核意见已提交!'
