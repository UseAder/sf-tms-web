<template>
    <div>
        <div class="J-main-body">
            ...
            <div class="J-main-search" >
              <el-form :inline="true" :model="searchForm" class="demo-form-inline" ref="searchForm">
                ...
                <el-form-item class="J-main-search-button">
                    <el-button type="primary" v-if="checkHasPermission('dbyfjs:read')" @click='handlesearch'>查询</el-button>
                    <el-button type="warning" v-if="checkHasPermission('dbyfjs:read')" @click="resetForm('searchForm')">重置</el-button>
                </el-form-item>
              </el-form>
            </div>
          </div>
          <div class="J-elTable" id="J-elTable" >
            <el-table  :max-height="getScreenHeight-jElTableOffsetTop>=111?getScreenHeight-jElTableOffsetTop:'none'"  :data="tableData" border @selection-change="handleSelectionChange" >
                  ...
                  <div class="table-menu">
                    <el-menu  :collapse="true"  >
                      <el-submenu index="1" >
                        ...
                        <el-menu-item-group>  
                          <el-menu-item index="1-1" v-if="scope.row.balanceStatus==1 && checkHasPermission('dbyfjs:update')" @click="showModifyBalance(scope.row)">修改对账单</el-menu-item>  
                          <el-menu-item index="1-2" v-if="scope.row.balanceStatus==1 && checkHasPermission('dbyfjs:delete')" @click="deleteBalance(scope.row)">删除对账单</el-menu-item> 
              ...
              <!-- <el-table-column prop="driverName" label="司机名称" width="180" align='center'></el-table-column> -->
              <el-table-column prop="drayageNum" label="短驳次数" width="180" align='center'></el-table-column>
              <el-table-column prop="sumDrayageFee" label="短驳费" width="180" align='center' class-name="J-expenditure"></el-table-column>
              ...
              <el-table-column prop="createEmployeeName" label="对账人"  align='center'></el-table-column>
        ...
        <dbCreateBalance @reloadList="reloadList" :pVisible.sync='modifyDialog.visible' pAuFlag='1' 
          :pBalanceId.sync='modifyDialog.balanceId'  :pBalanceNo.sync='modifyDialog.balanceNo'
          :pTruckId.sync='modifyDialog.truckId' :pPlateNumber.sync='modifyDialog.plateNumber' v-if="modifyDialog.visible"></dbCreateBalance>
...
import detailDbBalance from './detail.vue'
export default {
  mixins: [Base,List],
  components: { 
  ...
  methods: {
    ...
    querySpecialList() {//获取页面初始数据
      api.post('/finance/balance/drayage/balance/list/pc', this.searchForm).then(data=>{
          if(data.code==="200"){
    ...
    showModifyBalance(row){
      ...
      this.modifyDialog.truckId = row.truckId
      this.modifyDialog.plateNumber = row.plateNumber
    ...
    async showSubmitBalanceAudit(row) {
      ...
      //加载原审核流程信息
      const { code, message, data } = await api.get('/finance/balance/drayage/balance/audit/oldflow?id='+row.id)
      if(code=='200'){
    ...
    submitBalanceAudit() {
      ...
      api.post('/finance/balance/drayage/balance/audit/submit', this.auditForm).then(data=>{
        if(data.code==="200"){
    ...
    deleteBalance(row){
      this.$confirm('确定要删除对账单['+row.balanceNo+']吗?', '提示', {
        ...
        const { code, message, data } =await api.get('/finance/balance/drayage/balance/delete?id='+row.id)
        if(code==200){
          this.$message({
            type: 'success',
            message: '对账单删除成功!'
     ...
     exportFile(row){
      ...
      api.get('/finance/balance/drayage/export/pc/list?id='+row.id).then(data=>{
        if (data.code == '200') {
